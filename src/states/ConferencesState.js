...
export class ConferenceListener {
  ...
  constructor(sid, updateCallback) {
  ...
  initialize = async (sid) => {
    ...
    const syncClient = new TwilioSync(userToken);
    const syncMapName = `${sid}.CS`;
    let syncMap;
    let success = false;
    const maxRetries = 20;
    let retryCount = 0;
    ...
    while (!success && retryCount < maxRetries) {
      syncMap = await this.getSyncMap(syncClient, syncMapName);
      if (syncMap.sid) {
        success = true;
      } else {
        retryCount += 1;
...
export class ConferenceListenerManager {
  ...
  static startListening(task, consumerID) {
    console.debug(`Custom ConferenceListenerManager startListening ${task.sid} ${consumerID}`);
    ...
    if (!this.initialized) {
      ...
      this.requestedSids.set(task.sid, consumerID);
      return;
    }
    const wrapper = this.confs.get(task.sid);
    if (!wrapper) {
      console.debug('Custom ConferenceListenerManager new Listener');
      const listener = new ConferenceListener(task.sid, Actions.handleConferenceUpdate.bind(Actions));
      this.confs.set(task.sid, { listener, consumers: [consumerID] });
